...
class DoclingDocumentParser(BaseDocumentParser):
    ...
    def _parse_with_docling(
        ...
        # Fallback to max page_no from chunks if doc.pages is unreliable
        if page_count == 0 and chunks:
            page_count = max((c.page_no for c in chunks), default=1)
        
        # Ensure at least 1 page for non-empty documents
