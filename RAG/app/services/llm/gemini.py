    ...
    def _extract_with_thinking(response) -> LLMResult:
        ...
        if response.candidates:
            for part in response.candidates[0].content.parts:
                # Handle different SDK versions where thought might be string or bool
                thought_data = getattr(part, "thought", None)
                if thought_data:
                    if isinstance(thought_data, str):
                        thinking += thought_data
                    else:
        ...
        try:
            ...
            async for chunk in stream:
                ...
                for part in chunk.candidates[0].content.parts:
                    ...
                    thought_data = getattr(part, "thought", None)
                    if thought_data:
                        # Nếu thought_data là String (Gemini 2.0+), dùng nó luôn. Nếu là Bool, dùng part.text
                        thought_text = (thought_data if isinstance(thought_data, str) else getattr(part, "text", "")) or ""
                        if thought_text:
                            yield StreamChunk(type="thinking", text=thought_text)
                        ...
                        yield StreamChunk(type="text", text=part.text or "")
        except Exception as e:
